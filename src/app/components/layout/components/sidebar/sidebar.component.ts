  ...
  public getStudentTranscript(): void {
    this.apiService
      ...
      .subscribe((res) => {
        this.lessons = res;
        if (
          this.routerUrl == '/student_education' ||
          this.routerUrl == `/student_education?tkn=${this.token}`
        ) {
          this.homeService.changeContent(res[0].courseId);
        }
        if (
          this.routerUrl == '/student_education/lesson-info' ||
          this.routerUrl == `/student_education/lesson-info?tkn=${this.token}`
        ) {
